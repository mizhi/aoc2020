...
int main(int argc, char** argv) {
  ...
  /* With a 10-bit ID, there are 2**10 = 1024 Ids available. */
  int filled_seats[1024] = {0};
  while (fgets(buffer, 12, stdin) != NULL) {
    int current_id = decode_id(buffer);
    filled_seats[current_id] = 1;
  }
  ...
  for (int i = 0; i < 1024; i++) {
    if (filled_seats[i]) {
      ...
      /* when last_occupied is -1, then that means we haven't started finding
         occupied seats, which is what occurs at the start of the ID sequence
         where the rows don't exists on the aircraft. But if it is not -1, then
         that means we have seen occupied seats previously. This is where we
         break as this will be our seat. */
      if (last_occupied != -1) {
        printf("My seat is: %d\n", i);
        break;
      }
